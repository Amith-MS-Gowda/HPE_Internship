...
# Prometheus metrics
events_processed = Counter('wiki_events_processed_total', 'Number of events processed')
events_indexed = Counter('wiki_events_indexed_total', 'Number of events indexed to Elasticsearch')
index_errors = Counter('wiki_index_errors_total', 'Errors encountered during Elasticsearch indexing')
...
class WikiEvent(faust.Record, serializer='json'):
    ...
    user: str
    comment: str
    timestamp: int  # Raw UNIX timestamp
    bot: bool
    wiki: str
...
class Metrics(View):
    async def get(self, request: Request) -> Response:
        return self.text(generate_latest().decode('utf-8'), content_type=CONTENT_TYPE_LATEST)
